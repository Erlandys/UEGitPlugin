...
class FGitLFSSourceControlProvider : public ISourceControlProvider
{
public:
	...
	inline const FGitLFSVersion& GetGitVersion() const
	{
		return GitVersion;
	}
	...
	TSharedRef<FGitLFSSourceControlState, ESPMode::ThreadSafe> GetStateInternal(const FString& Filename);

	/** Helper function used to update changelists state cache */
	TSharedRef<FGitLFSSourceControlChangelistState, ESPMode::ThreadSafe> GetStateInternal(const FGitLFSSourceControlChangelist& InChangelist);
...
private:
	...
	TSharedPtr<class IGitLFSSourceControlWorker, ESPMode::ThreadSafe> CreateWorker(const FName& InOperationName) const;

	/** Helper function for running command synchronously. */
	ECommandResult::Type ExecuteSynchronousCommand(class FGitLFSSourceControlCommand& InCommand, const FText& Task, bool bSuppressResponseMsg);
	/** Issue a command asynchronously if possible. */
	ECommandResult::Type IssueCommand(class FGitLFSSourceControlCommand& InCommand, const bool bSynchronous = false );

	/** Output any messages this command holds */
	void OutputCommandMessages(const class FGitLFSSourceControlCommand& InCommand) const;

	/** Update repository status on Connect and UpdateStatus operations */
	void UpdateRepositoryStatus(const class FGitLFSSourceControlCommand& InCommand);
	...
	/** State cache */
	TMap<FString, TSharedRef<class FGitLFSSourceControlState, ESPMode::ThreadSafe> > StateCache;
	TMap<FGitLFSSourceControlChangelist, TSharedRef<class FGitLFSSourceControlChangelistState, ESPMode::ThreadSafe> > ChangelistsStateCache;
	...
	TArray < FGitLFSSourceControlCommand* > CommandQueue;
	...
	FGitLFSSourceControlMenu GitSourceControlMenu;
	...
	*/
	...
	class FGitLFSSourceControlRunner* Runner = nullptr;
};
