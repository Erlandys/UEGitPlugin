...
void FGitLFSLockedFilesCache::OnFileLockChanged(const FString& filePath, const FString& lockUser, bool locked)
{
	const FString& LfsUserName = FGitLFSSourceControlModule::Get().GetProvider().GetLockUser();
...
namespace GitLFSSourceControlUtils
{
...
bool CheckGitAvailability(const FString& InPathToGitBinary, FGitLFSVersion* OutVersion)
{
	FString InfoMessages;
	FString ErrorMessages;
	bool bGitAvailable = RunCommandInternalRaw(TEXT("version"), InPathToGitBinary, FString(), FGitLFSSourceControlModule::GetEmptyStringArray(), FGitLFSSourceControlModule::GetEmptyStringArray(), InfoMessages, ErrorMessages);
	if (bGitAvailable)
	{
...
void ParseGitVersion(const FString& InVersionString, FGitLFSVersion* OutVersion)
{
#if UE_BUILD_DEBUG
...
void GetUserConfig(const FString& InPathToGitBinary, const FString& InRepositoryRoot, FString& OutUserName, FString& OutUserEmail)
{
	...
	bResults = RunCommandInternal(TEXT("config"), InPathToGitBinary, InRepositoryRoot, Parameters, FGitLFSSourceControlModule::GetEmptyStringArray(), InfoMessages, ErrorMessages);
...
bool GetBranchName(const FString& InPathToGitBinary, const FString& InRepositoryRoot, FString& OutBranchName)
{
	...
	bResults = RunCommand(TEXT("symbolic-ref"), InPathToGitBinary, InRepositoryRoot, Parameters, FGitLFSSourceControlModule::GetEmptyStringArray(), InfoMessages, ErrorMessages);
	if (bResults && InfoMessages.Num() > 0)
	{
	...
	else
	{
		...
		bResults = RunCommand(TEXT("log"), InPathToGitBinary, InRepositoryRoot, Parameters, FGitLFSSourceControlModule::GetEmptyStringArray(), InfoMessages, ErrorMessages);
		if (bResults && InfoMessages.Num() > 0)
		{
...
bool GetRemoteBranchName(const FString& InPathToGitBinary, const FString& InRepositoryRoot, FString& OutBranchName)
{
	...
	bool bResults = RunCommand(TEXT("rev-parse"), InPathToGitBinary, InRepositoryRoot, Parameters, FGitLFSSourceControlModule::GetEmptyStringArray(),
...
bool GetCommitInfo(const FString& InPathToGitBinary, const FString& InRepositoryRoot, FString& OutCommitId, FString& OutCommitSummary)
{
	...
	bResults = RunCommandInternal(TEXT("log"), InPathToGitBinary, InRepositoryRoot, Parameters, FGitLFSSourceControlModule::GetEmptyStringArray(), InfoMessages, ErrorMessages);
	if (bResults && InfoMessages.Num() > 0)
	{
...
bool GetRemoteUrl(const FString& InPathToGitBinary, const FString& InRepositoryRoot, FString& OutRemoteUrl)
{
	...
	const bool bResults = RunCommandInternal(TEXT("remote"), InPathToGitBinary, InRepositoryRoot, Parameters, FGitLFSSourceControlModule::GetEmptyStringArray(), InfoMessages, ErrorMessages);
	if (bResults && InfoMessages.Num() > 0)
	{
				   ...
				   TArray<FString>& OutResults, TArray<FString>& OutErrorMessages)
{
	...
	FString BaseDir = IPluginManager::Get().FindPlugin("GitLFSSourceControl")->GetBaseDir();
#if PLATFORM_WINDOWS
	...
	return GitLFSSourceControlUtils::RunCommand(Command, LFSLockBinary, InRepositoryRoot, InParameters, InFiles, OutResults, OutErrorMessages);
}
...
public:
	FGitLfsLocksParser(const FString& InRepositoryRoot, const FString& InStatus, const bool bAbsolutePaths = true)
	{
		...
		if (Informations.Num() >= 2)
		{
			...
			{
				// TODO: thread safety
				LockUser = FGitLFSSourceControlModule::Get().GetProvider().GetLockUser();
			}
...
public:
	FGitStatusParser(const FString& InResult)
	{
		...
		if ((IndexState == 'U' || WCopyState == 'U') || (IndexState == 'A' && WCopyState == 'A') || (IndexState == 'D' && WCopyState == 'D'))
		{
			...
			FileState = EGitLFSFileState::Unmerged;
			TreeState = EGitLFSTreeState::Working;
			return;
		}
		...
		{
			TreeState = EGitLFSTreeState::Staged;
		}

		if (IndexState == '?' || WCopyState == '?')
		{
			TreeState = EGitLFSTreeState::Untracked;
			FileState = EGitLFSFileState::Unknown;
		}
		else if (IndexState == '!' || WCopyState == '!')
		{
			TreeState = EGitLFSTreeState::Ignored;
			FileState = EGitLFSFileState::Unknown;
		}
		else if (IndexState == 'A')
		{
			FileState = EGitLFSFileState::Added;
		}
		else if (IndexState == 'D')
		{
			FileState = EGitLFSFileState::Deleted;
		}
		else if (WCopyState == 'D')
		{
			FileState = EGitLFSFileState::Missing;
		}
		else if (IndexState == 'M' || WCopyState == 'M')
		{
			FileState = EGitLFSFileState::Modified;
		}
		else if (IndexState == 'R')
		{
			FileState = EGitLFSFileState::Renamed;
		}
		else if (IndexState == 'C')
		{
			FileState = EGitLFSFileState::Copied;
		}
		else
		{
	...
	EGitLFSFileState::Type FileState;
	EGitLFSTreeState::Type TreeState;
};
...
static void RunGetConflictStatus(const FString& InPathToGitBinary, const FString& InRepositoryRoot, const FString& InFile, FGitLFSSourceControlState& InOutFileState)
{
	TArray<FString> ErrorMessages;
...
bool ListFilesInDirectoryRecurse(const FString& InPathToGitBinary, const FString& InRepositoryRoot, const FString& InDirectory, TArray<FString>& OutFiles)
{
	...
	const bool bResult = RunCommandInternal(TEXT("ls-files"), InPathToGitBinary, InRepositoryRoot, FGitLFSSourceControlModule::GetEmptyStringArray(), Directory, OutFiles, ErrorMessages);
 ...
 */
static void ParseDirectoryStatusResult(const bool InUsingLfsLocking, const TMap<FString, FString>& InResults, TMap<FString, FGitLFSSourceControlState>& OutStates)
{
	// Iterate on each line of result of the status command
	for (const auto& Result : InResults)
	{
		FGitLFSSourceControlState FileState(Result.Key);
		if (!InUsingLfsLocking)
		{
			FileState.State.LockState = EGitLFSLockState::Unlockable;
		}
		FGitStatusParser StatusParser(Result.Value);
		if ((EGitLFSFileState::Deleted == StatusParser.FileState) || (EGitLFSFileState::Missing == StatusParser.FileState) || (EGitLFSTreeState::Untracked == StatusParser.TreeState))
		{
...
static void ParseFileStatusResult(const FString& InPathToGitBinary, const FString& InRepositoryRoot, const bool InUsingLfsLocking, const TSet<FString>& InFiles,
								  const TMap<FString, FString>& InResults, TMap<FString, FGitLFSSourceControlState>& OutStates)
{
	...
	for (const auto& File : InFiles)
	{
		FGitLFSSourceControlState FileState(File);
		FileState.State.FileState = EGitLFSFileState::Unset;
		FileState.State.TreeState = EGitLFSTreeState::Unset;
		FileState.State.LockState = EGitLFSLockState::Unset;
		...
		{
			FileState.State.FileState = EGitLFSFileState::Unknown;
			// File not found in status
			if (FPaths::FileExists(File))
			{
				// usually means the file is unchanged,
				FileState.State.TreeState = EGitLFSTreeState::Unmodified;
#if UE_BUILD_DEBUG && GIT_DEBUG_STATUS
			...
			else
			{
				...
				FileState.State.TreeState = EGitLFSTreeState::NotInRepo;
#if UE_BUILD_DEBUG && GIT_DEBUG_STATUS
		...
		else
		{
			if (IsFileLFSLockable(File))
			{
				...
				if (LockedFiles.Contains(File))
				{
					...
					{
						FileState.State.LockState = EGitLFSLockState::Locked;
					}
					else
					{
						FileState.State.LockState = EGitLFSLockState::LockedOther;
					}
				}
				else
				{
					FileState.State.LockState = EGitLFSLockState::NotLocked;
#if UE_BUILD_DEBUG && GIT_DEBUG_STATUS
...
void CheckRemote(const FString& InPathToGitBinary, const FString& InRepositoryRoot, const TArray<FString>& Files,
				 TArray<FString>& OutErrorMessages, TMap<FString, FGitLFSSourceControlState>& OutStates)
{
	...
	{
		if (FGitLFSSourceControlState* FileState = OutStates.Find(NewFile.Key))
		{
			FileState->State.RemoteState = NewFile.Value.Equals(CurrentBranchName) ? EGitLFSRemoteState::NotAtHead : EGitLFSRemoteState::NotLatest;
...
bool GetAllLocks(const FString& InRepositoryRoot, const FString& GitBinaryFallback, TArray<FString>& OutErrorMessages, TMap<FString, FString>& OutLocks, bool bInvalidateCache)
{
	...
	if (!bInvalidateCache)
	{
		const FTimespan CacheTimeElapsed = CurrentTime - FGitLFSLockedFilesCache::LastUpdated;
	...
	if (bCacheExpired)
	{
		...
		TArray<FString> ErrorMessages;
		TArray<FString> Results;
		bResult = RunLFSCommand(TEXT("locks"), InRepositoryRoot, GitBinaryFallback, FGitLFSSourceControlModule::GetEmptyStringArray(), FGitLFSSourceControlModule::GetEmptyStringArray(),
		...
		if (bResult)
		{
			...
			FGitLFSLockedFilesCache::LastUpdated = CurrentTime;
			FGitLFSLockedFilesCache::SetLockedFiles(OutLocks);
			return bResult;
		}
		...
		else
		{
			...
			Results.Reset();
			bResult = RunLFSCommand(TEXT("locks"), InRepositoryRoot, GitBinaryFallback, Params, FGitLFSSourceControlModule::GetEmptyStringArray(), Results, OutErrorMessages);
			for (const FString& Result : Results)
			{
				FGitLfsLocksParser LockFile(InRepositoryRoot, Result);
	#if UE_BUILD_DEBUG && GIT_DEBUG_STATUS
	...
	if (!bResult)
	{
		...
		OutLocks = FGitLFSLockedFilesCache::GetLockedFiles();
		bResult = true;
	}
	return bResult;
}
...
void GetLockedFiles(const TArray<FString>& InFiles, TArray<FString>& OutFiles)
{
	...
	{
		const auto& GitState = StaticCastSharedRef<FGitLFSSourceControlState>(State);
...
bool UpdateChangelistStateByCommand()
{
	...
	TSharedRef<FGitLFSSourceControlChangelistState, ESPMode::ThreadSafe> StagedChangelist = Provider.GetStateInternal(FGitLFSSourceControlChangelist::StagedChangelist);
	TSharedRef<FGitLFSSourceControlChangelistState, ESPMode::ThreadSafe> WorkingChangelist = Provider.GetStateInternal(FGitLFSSourceControlChangelist::WorkingChangelist);
	...
	for (const auto& Result : Results)
	{
		...
		TSharedRef<FGitLFSSourceControlState, ESPMode::ThreadSafe> State = Provider.GetStateInternal(File);
		// Staged check
...
bool UpdateFileStagingOnSavedInternal(const FString& Filename)
{
	...
	TSharedRef<FGitLFSSourceControlState, ESPMode::ThreadSafe> State = Provider.GetStateInternal(Filename);
	...
	if (State->Changelist.GetName().Equals(TEXT("Staged")))
	{
		...
		bResult = RunCommand(TEXT("add"), Provider.GetGitBinaryPath(), Provider.GetPathToRepositoryRoot(), FGitLFSSourceControlModule::GetEmptyStringArray(), File, DummyResults, DummyMsgs);
	}
...
void UpdateStateOnAssetRename(const FAssetData& InAssetData, const FString& InOldName)
{
	...
	TSharedRef<FGitLFSSourceControlState, ESPMode::ThreadSafe> State = Provider.GetStateInternal(InOldName);	
...
{
	TSharedRef<FGitLFSSourceControlRevision, ESPMode::ThreadSafe> SourceControlRevision = MakeShareable(new FGitLFSSourceControlRevision);
	for (const auto& Result : InResults)
	{
		if (Result.StartsWith(TEXT("commit "))) // Start of a new commit
		{
			...
			if (SourceControlRevision->RevisionNumber != 0)
			{
				...
				SourceControlRevision = MakeShareable(new FGitLFSSourceControlRevision);
			}
...
bool UpdateCachedStates(const TMap<const FString, FGitLFSState>& InResults)
{
	if (InResults.Num() == 0)
	{
		return false;
	...
	{
		TSharedRef<FGitLFSSourceControlState, ESPMode::ThreadSafe> State = Provider.GetStateInternal(Pair.Key);
		const FGitLFSState& NewState = Pair.Value;
		if (NewState.FileState != EGitLFSFileState::Unset)
		{
			// Invalid transition
			if (NewState.FileState == EGitLFSFileState::Added && !State->IsUnknown() && !State->CanAdd())
			{
				continue;
			}
		...
		if (NewState.RemoteState != EGitLFSRemoteState::Unset)
		{
			...
			if (NewState.RemoteState == EGitLFSRemoteState::UpToDate)
			{
...
bool CollectNewStates(const TMap<FString, FGitLFSSourceControlState>& InStates, TMap<const FString, FGitLFSState>& OutResults)
{
	if (InStates.Num() == 0)
	{
		return false;
...
bool CollectNewStates(const TArray<FString>& InFiles, TMap<const FString, FGitLFSState>& OutResults, EGitLFSFileState::Type FileState, EGitLFSTreeState::Type TreeState, EGitLFSLockState::Type LockState, EGitLFSRemoteState::Type RemoteState)
{
	if (InFiles.Num() == 0)
	{
		return false;
...
void RemoveRedundantErrors(FGitLFSSourceControlCommand& InCommand, const FString& InFilter)
{
...
bool FetchRemote(const FString& InPathToGitBinary, const FString& InPathToRepositoryRoot, bool InUsingGitLfsLocking, TArray<FString>& OutResults, TArray<FString>& OutErrorMessages)
{
	...
	return RunCommand(TEXT("fetch"), InPathToGitBinary, InPathToRepositoryRoot, Params,
					  FGitLFSSourceControlModule::GetEmptyStringArray(), OutResults, OutErrorMessages);
}
...
bool PullOrigin(const FString& InPathToGitBinary, const FString& InPathToRepositoryRoot, const TArray<FString>& InFiles, TArray<FString>& OutFiles,
				TArray<FString>& OutResults, TArray<FString>& OutErrorMessages)
{
	if (FGitLFSSourceControlModule::Get().GetProvider().bPendingRestart)
	{
	...
	const bool bResultDiff = RunCommand(TEXT("diff"), InPathToGitBinary, InPathToRepositoryRoot, { TEXT("--name-only"), RemoteBranch }, FGitLFSSourceControlModule::GetEmptyStringArray(), DifferentFiles, OutErrorMessages);
	if (!bResultDiff)
	{
		return false;
	}
	...
	TArray<FString> InfoMessages;
	bool bSuccess = RunCommand(TEXT("pull"), InPathToGitBinary, InPathToRepositoryRoot, { "--rebase", "--autostash" }, FGitLFSSourceControlModule::GetEmptyStringArray(),
