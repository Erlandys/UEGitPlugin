...
void FGitLFSSourceControlProvider::Init(bool bForceConnection)
{
	...
	if(!bGitAvailable)
	{
		const TSharedPtr<IPlugin> Plugin = IPluginManager::Get().FindPlugin(TEXT("GitLFSSourceControl"));
		if(Plugin.IsValid())
		{
	...
	UPackage::PackageSavedWithContextEvent.AddStatic(&GitLFSSourceControlUtils::UpdateFileStagingOnSaved);
	
	FAssetRegistryModule& AssetRegistryModule = FModuleManager::LoadModuleChecked<FAssetRegistryModule>(TEXT("AssetRegistry"));
	AssetRegistryModule.Get().OnAssetRenamed().AddStatic(&GitLFSSourceControlUtils::UpdateStateOnAssetRename);	
...
void FGitLFSSourceControlProvider::CheckGitAvailability()
{
	...
	if(PathToGitBinary.IsEmpty())
	{
		...
		PathToGitBinary = GitLFSSourceControlUtils::FindGitBinaryPath();
		if(!PathToGitBinary.IsEmpty())
		{
...
void FGitLFSSourceControlProvider::CheckRepositoryStatus()
{
	...
	if (!GitLFSSourceControlUtils::FindRootDirectory(PathToProjectDir, PathToGitRoot))
	{
	...
	if (!GitLFSSourceControlUtils::CheckGitAvailability(PathToGitBinary, &GitVersion))
	{
	...
	TUniqueFunction<void()> InitFunc = [this]()
	{
		if (!IsInGameThread())
		{
			...
			do
			{
				GitModule = FModuleManager::Get().GetModule("GitLFSSourceControl");
				FPlatformProcess::Sleep(0.0f);
			} while (!GitModule);
		}

		// Get user name & email (of the repository, else from the global Git config)
		GitLFSSourceControlUtils::GetUserConfig(PathToGitBinary, PathToRepositoryRoot, UserName, UserEmail);
		
		TMap<FString, FGitLFSSourceControlState> States;
		auto ConditionalRepoInit = [this, &States]()
		{
			if (!GitLFSSourceControlUtils::GetBranchName(PathToGitBinary, PathToRepositoryRoot, BranchName))
			{
				return false;
			}
			GitLFSSourceControlUtils::GetRemoteBranchName(PathToGitBinary, PathToRepositoryRoot, RemoteBranchName);
			GitLFSSourceControlUtils::GetRemoteUrl(PathToGitBinary, PathToRepositoryRoot, RemoteUrl);
			const TArray<FString> Files{TEXT("*.uasset"), TEXT("*.umap")};
			TArray<FString> LockableErrorMessages;
			if (!GitLFSSourceControlUtils::CheckLFSLockable(PathToGitBinary, PathToRepositoryRoot, Files, LockableErrorMessages))
			{
			...
			if (!GitLFSSourceControlUtils::RunUpdateStatus(PathToGitBinary, PathToRepositoryRoot, bUsingGitLfsLocking, ProjectDirs, StatusErrorMessages, States))
			{
				return false;
			}
			return true;
		...
		if (ConditionalRepoInit())
		{
			...
			{
				TMap<const FString, FGitLFSState> Results;
				if (GitLFSSourceControlUtils::CollectNewStates(States, Results))
				{
					GitLFSSourceControlUtils::UpdateCachedStates(Results);
				}
				Runner = new FGitLFSSourceControlRunner();
...
TSharedRef<FGitLFSSourceControlState, ESPMode::ThreadSafe> FGitLFSSourceControlProvider::GetStateInternal(const FString& Filename)
{
	TSharedRef<FGitLFSSourceControlState, ESPMode::ThreadSafe>* State = StateCache.Find(Filename);
	if (State != NULL)
	{
		// found cached item
		return (*State);
	...
	else
	{
		...
		TSharedRef<FGitLFSSourceControlState, ESPMode::ThreadSafe> NewState = MakeShareable( new FGitLFSSourceControlState(Filename) );
		StateCache.Add(Filename, NewState);
		return NewState;
	}
}

TSharedRef<FGitLFSSourceControlChangelistState, ESPMode::ThreadSafe> FGitLFSSourceControlProvider::GetStateInternal(const FGitLFSSourceControlChangelist& InChangelist)
{
	TSharedRef<FGitLFSSourceControlChangelistState, ESPMode::ThreadSafe>* State = ChangelistsStateCache.Find(InChangelist);
	if (State != NULL)
	{
		// found cached item
		return (*State);
	...
	{
		// cache an unknown state for this item
		TSharedRef<FGitLFSSourceControlChangelistState, ESPMode::ThreadSafe> NewState = MakeShared<FGitLFSSourceControlChangelistState>(InChangelist);
...
bool FGitLFSSourceControlProvider::IsEnabled() const
{
	return bGitRepositoryFound;
}

/** Quick check if revision control is available for use (useful for server-based providers) */
bool FGitLFSSourceControlProvider::IsAvailable() const
{
	return bGitRepositoryFound;
}

const FName& FGitLFSSourceControlProvider::GetName(void) const
{
	return ProviderName;
}

ECommandResult::Type FGitLFSSourceControlProvider::GetState( const TArray<FString>& InFiles, TArray< TSharedRef<ISourceControlState, ESPMode::ThreadSafe> >& OutState, EStateCacheUsage::Type InStateCacheUsage )
{
	if (!IsEnabled())
	{
...
ECommandResult::Type FGitLFSSourceControlProvider::GetState(const TArray<FSourceControlChangelistRef>& InChangelists, TArray<FSourceControlChangelistStateRef>& OutState, EStateCacheUsage::Type InStateCacheUsage)
{
	...
	{
		FGitSourceControlChangelistRef GitChangelist = StaticCastSharedRef<FGitLFSSourceControlChangelist>(Changelist);
...
FDelegateHandle FGitLFSSourceControlProvider::RegisterSourceControlStateChanged_Handle( const FSourceControlStateChanged::FDelegate& SourceControlStateChanged )
{
	return OnSourceControlStateChanged.Add( SourceControlStateChanged );
}

void FGitLFSSourceControlProvider::UnregisterSourceControlStateChanged_Handle( FDelegateHandle Handle )
{
	OnSourceControlStateChanged.Remove( Handle );
}

#if ENGINE_MAJOR_VERSION < 5
ECommandResult::Type FGitLFSSourceControlProvider::Execute( const FSourceControlOperationRef& InOperation, const TArray<FString>& InFiles, EConcurrency::Type InConcurrency, const FSourceControlOperationComplete& InOperationCompleteDelegate )
#else
ECommandResult::Type FGitLFSSourceControlProvider::Execute( const FSourceControlOperationRef& InOperation, FSourceControlChangelistPtr InChangelist, const TArray<FString>& InFiles, EConcurrency::Type InConcurrency, const FSourceControlOperationComplete& InOperationCompleteDelegate )
#endif
{
	...
	TSharedPtr<IGitLFSSourceControlWorker, ESPMode::ThreadSafe> Worker = CreateWorker(InOperation->GetName());
	if(!Worker.IsValid())
	{
	...
	FGitLFSSourceControlCommand* Command = new FGitLFSSourceControlCommand(InOperation, Worker.ToSharedRef());
	Command->Files = AbsoluteFiles;
	Command->UpdateRepositoryRootIfSubmodule(AbsoluteFiles);
	Command->OperationCompleteDelegate = InOperationCompleteDelegate;

	TSharedPtr<FGitLFSSourceControlChangelist, ESPMode::ThreadSafe> ChangelistPtr = StaticCastSharedPtr<FGitLFSSourceControlChangelist>(InChangelist);
...
#endif
{
	for (int32 CommandIndex = 0; CommandIndex < CommandQueue.Num(); ++CommandIndex)
	{
		FGitLFSSourceControlCommand& Command = *CommandQueue[CommandIndex];
		if (Command.Operation == InOperation)
		{
			check(Command.bAutoDelete);
			Command.Cancel();
...
bool FGitLFSSourceControlProvider::UsesLocalReadOnlyState() const
{
	return bUsingGitLfsLocking; // Git LFS Lock uses read-only state
}

bool FGitLFSSourceControlProvider::UsesChangelists() const
{
	return true;
}

bool FGitLFSSourceControlProvider::UsesCheckout() const
{
	return bUsingGitLfsLocking; // Git LFS Lock uses read-only state
}

#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 1
bool FGitLFSSourceControlProvider::UsesFileRevisions() const
{
	return true;
}

TOptional<bool> FGitLFSSourceControlProvider::IsAtLatestRevision() const
{
	return TOptional<bool>();
}

TOptional<int> FGitLFSSourceControlProvider::GetNumLocalChanges() const
{
...
bool FGitLFSSourceControlProvider::AllowsDiffAgainstDepot() const
{
	return true;
}

bool FGitLFSSourceControlProvider::UsesUncontrolledChangelists() const
{
	return true;
}

bool FGitLFSSourceControlProvider::UsesSnapshots() const
{
	return false;
}
#endif
...
bool FGitLFSSourceControlProvider::CanExecuteOperation(const FSourceControlOperationRef& InOperation) const {
	return WorkersMap.Find(InOperation->GetName()) != nullptr;
}

TMap<ISourceControlProvider::EStatus, FString> FGitLFSSourceControlProvider::GetStatus() const
{
...
TSharedPtr<IGitLFSSourceControlWorker, ESPMode::ThreadSafe> FGitLFSSourceControlProvider::CreateWorker(const FName& InOperationName) const
{
...
void FGitLFSSourceControlProvider::RegisterWorker( const FName& InName, const FGetGitSourceControlWorker& InDelegate )
{
	WorkersMap.Add( InName, InDelegate );
}

void FGitLFSSourceControlProvider::OutputCommandMessages(const FGitLFSSourceControlCommand& InCommand) const
{
...
void FGitLFSSourceControlProvider::UpdateRepositoryStatus(const class FGitLFSSourceControlCommand& InCommand)
{
...
TArray< TSharedRef<ISourceControlLabel> > FGitLFSSourceControlProvider::GetLabels( const FString& InMatchingSpec ) const
{
...
TArray<FSourceControlChangelistRef> FGitLFSSourceControlProvider::GetChangelists( EStateCacheUsage::Type InStateCacheUsage )
{
	...
	Algo::Transform(ChangelistsStateCache, Changelists, [](const auto& Pair) { return MakeShared<FGitLFSSourceControlChangelist, ESPMode::ThreadSafe>(Pair.Key); });
	return Changelists;
}
#endif

#if SOURCE_CONTROL_WITH_SLATE
TSharedRef<class SWidget> FGitLFSSourceControlProvider::MakeSettingsWidget() const
{
	return SNew(SGitLFSSourceControlSettings);
}
#endif

ECommandResult::Type FGitLFSSourceControlProvider::ExecuteSynchronousCommand(FGitLFSSourceControlCommand& InCommand, const FText& Task, bool bSuppressResponseMsg)
{
	ECommandResult::Type Result = ECommandResult::Failed;

	struct Local
	{
		static void CancelCommand(FGitLFSSourceControlCommand* InControlCommand)
		{
...
ECommandResult::Type FGitLFSSourceControlProvider::IssueCommand(FGitLFSSourceControlCommand& InCommand, const bool bSynchronous)
{
...
void FGitLFSSourceControlProvider::RegisterStateBranches(const TArray<FString>& BranchNames, const FString& ContentRootIn)
{
...
TArray<FString> FGitLFSSourceControlProvider::GetStatusBranchNames() const
{
	...
	for (int i = 0; i < StatusBranchNamePatternsInternal.Num(); i++)
	{
		...
		bool bResult = GitLFSSourceControlUtils::GetRemoteBranchesWildcard(PathToGitBinary, PathToRepositoryRoot, StatusBranchNamePatternsInternal[i], Matches);
